...
class TelephonicAuditController extends Controller
{
    public function viewAudits() {
        $query = PmjayTreatment::with('hospital');
        $audits = $query->latest()->paginate(100);
        return view('dmo.audits.telephonic.view_all_audits', compact('audits'));
    }
    ...
    public function storeTelephonicObservation(Request $request, $auditId)
    {
        ...
        if (TelephonicAudit::where('pmjay_treatment_id', $auditId)->exists()) {
            return back()->with('error','Observation already submitted.');
        }

        TelephonicAudit::create([
            'pmjay_treatment_id' => $audit->id,
