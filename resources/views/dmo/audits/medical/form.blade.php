...
<div class="mb-7">
    ...
    <p class="text-sm text-slate-500 mt-1">
        Medical Audit · PMJAY Assam audit
    </p>
</div>
...
@php $isCompleted = $audit->status === 'completed_1'; @endphp
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden mb-6">
    ...
    <div class="p-4 md:p-6 grid grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
        <div class="bg-slate-50/70 p-3 rounded-xl border border-slate-100">
            ...
            <span class="font-semibold text-slate-800 break-all">{{ $audit->registration_id }}</span>
        </div>
        <div class="bg-slate-50/70 p-3 rounded-xl border border-slate-100">
            <span class="text-slate-400 text-xs block mb-0.5">Case ID</span>
            <span class="font-semibold text-slate-800 break-all">{{ $audit->case_id }}</span>
        </div>
        ...
        <div class="bg-slate-50/70 p-3 rounded-xl border border-slate-100">
            ...
            <span class="font-semibold text-slate-800">{{ $audit->ben_mobile_no }}</span>
        </div>
        <div class="bg-slate-50/70 p-3 rounded-xl border border-slate-100">
            <span class="text-slate-400 text-xs block mb-0.5">Hospital</span>
            <span class="font-semibold text-slate-800">{{ $audit->hospital->name }}</span>
        </div>
        <div class="bg-slate-50/70 p-3 rounded-xl border border-slate-100">
            <span class="text-slate-400 text-xs block mb-0.5">District</span>
            <span class="font-semibold text-slate-800">{{ $audit->patientDistrict->name }}</span>
        </div>
        <div class="bg-slate-50/70 p-3 rounded-xl border border-slate-100 col-span-2 lg:col-span-2">
            ...
            <span class="font-semibold text-slate-800">{{ $audit->procedure_details }}</span>
        </div>
        <div class="bg-emerald-50 p-3 rounded-xl border border-emerald-100 col-span-2 md:col-span-1">
            ...
            <span class="font-bold text-emerald-700 text-base">₹ {{ $audit->amount_preauth_approved }}</span>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-md p-4 md:p-8">
    ...
    <form id="medicalAuditForm" method="POST" action="{{ route('dmo.audits.medical.store', $audit->id) }}" enctype="multipart/form-data">
        @csrf
...
(function () {
    ...
    const form       = document.getElementById('medicalAuditForm');
    ...
    /* ═══ Surgical-only field visibility ═══ */
    window.applySurgicalVisibility = function () {
        const isSurgical = _selectedTT === 'Surgical';
        document.querySelectorAll('.surgical-row').forEach(el => {
            el.style.display = isSurgical ? 'flex' : 'none';
        });
    };
    // run once on load in case old() pre-selected a value
    if (_selectedTT) applySurgicalVisibility();
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!_selectedTT) {
            toast('error', 'Treatment Type Required', 'Please select whether this was a Surgical or Medical treatment.');
            document.getElementById('treatment-type-group').scrollIntoView({ behavior: 'smooth', block: 'center' });
            blocked = true;
        }
