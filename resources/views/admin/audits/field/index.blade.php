...
<div class="flex flex-wrap items-center justify-between gap-3 mb-7">
    <div>
        <div class="flex items-center gap-2 text-sm text-slate-400 mb-1">
            ...
            <span class="text-slate-600 font-medium">Field Visits</span>
        </div>
        <h1 class="text-2xl md:text-3xl font-black text-slate-900 tracking-tight" style="font-family:'Syne',sans-serif;">
            Field Visits
        </h1>
        <p class="text-sm text-slate-500 mt-1">Physical hospital verification visits · admin view</p>
    </div>
</div>
@endsection
...
<style>
    .page-card::before { content:''; display:block; height:3px; background:linear-gradient(90deg,#f59e0b,#f97316); }
</style>
@endsection
...
<form method="GET" action="{{ route('admin.audits.field.index') }}" class="filter-bar">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">{{ $audit->treatment?->ben_mobile_no ?? '' }}</p>
                    </td>
                    ...
                    <td class="text-slate-500 text-xs">{{ $audit->district?->name ?? '—' }}</td>
                    <td class="text-slate-500 text-xs max-w-[160px] truncate" title="{{ $audit->treatment?->procedure_details }}">
                        {{ $audit->treatment?->procedure_details ?? '—' }}
                    </td>
                    <td class="text-emerald-700 font-semibold text-xs">
                        {{ $audit->treatment?->amount_preauth_approved ? '₹ '.number_format($audit->treatment->amount_preauth_approved) : '—' }}
                    </td>
                    <td class="text-slate-600 text-xs">{{ $audit->fieldVisit?->submittedBy?->name ?? '—' }}</td>
                    <td>
                    ...
                    <td>
                        <a href="{{ route('admin.audits.field.show', $audit->id) }}" class="action-btn" title="View">
                            <i class="fas fa-eye text-xs"></i>
                        </a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="10"><div class="empty-state"><i class="fas fa-hospital"></i><p>No field visits found.</p></div></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
